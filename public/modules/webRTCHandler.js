...
export function startWebRTCProces() {
    ...
    DOM.offeror.offerorCreatePcButton.addEventListener("click", async () => {
    ...
    DOM.offeror.offerorAddDataTypeButton.addEventListener("click", async () => {
        // create our dataChannel
        createDataChannel(true);
        await new Promise(resolve => setTimeout(resolve, 100));
        // update the video and stream the data
        const mediaStream = await uiUtils.startVideoStream();
...
async function createPeerConnectionObject() {
    ...
    // stream the media track
    pc.ontrack = event => {
        console.log("STREAM EVENTS: ", event?.streams);
        if (event.streams && event.streams?.[0]) {
            uiUtils.streamVideo(event.streams[0]);
        }
    }
...
export function handleOffer(data) {
    ...
    DOM.offeree.offereeCreatePcButton.addEventListener("click", async () => {
        await createPeerConnectionObject(); 
        // ui changes
    ...
    DOM.offeree.offereeAddDataTypeButton.addEventListener("click", async () => {
        ...
        console.log("TRANSCEIVERS ANSWER: ", pc.getTransceivers());
        console.log("SENDERS ANSWER SECTION: ", pc.getSenders());
        // ui changes
