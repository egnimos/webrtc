...
export async function startVideoStream() {
    try {
        const videoFeed = await navigator.mediaDevices.getUserMedia({video: true, audio: true});
        console.log(videoFeed.getTracks());
        return videoFeed;
    } catch (error) {
        console.log("Error occured while starting the video stream: ", error);
        return null;
    }
}

export function streamVideo(stream) {
    try {
        myVideoElement.srcObject = stream;
    } catch (error) {
        console.log("Failed to stream the video: ", error);
    }
}
